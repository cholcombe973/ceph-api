...
class MdsCommand:
    ...
    def mds_repaired(self, rank):
        ...
        if not isinstance(rank, six.string_types):
    ...
    def fs_set(self, var, fs_name, val, confirm=None):
        ...
        if not isinstance(fs_name, six.string_types):
            raise TypeError("fs_name is not a String")
    ...
    def fs_rm_data_pool(self, pool, fs_name):
        """
        remove data pool <pool>
        ...
        if not isinstance(fs_name, six.string_types):
            raise TypeError("fs_name is not a String")
        cmd = {'prefix': 'fs rm_data_pool', 'pool': pool, 'fs_name': fs_name}
        return run_ceph_command(self.rados_config_file, cmd, inbuf='')
...
class OsdCommand:
    ...
    def osd_map(self, object, pool, nspace=None):
        ...
        if nspace is not None:
            if not isinstance(nspace, six.string_types):
                raise TypeError("nspace is not a String")
            cmd['nspace'] = nspace
        return run_ceph_command(self.rados_config_file, cmd, inbuf='')
    ...
    def osd_crush_add(self, weight, id, args):
        """
    ...
    def osd_crush_tunables(self, profile):
        ...
        validator(value=profile,
                  ...
                  valid_range=["legacy", "argonaut", "bobtail", "firefly",
                               "hammer", "jewel", "optimal", "default"]), str(
    ...
    def osd_create(self, id=None, uuid=None):
        ...
        if id is not None:
            if not isinstance(id, six.integer_types):
                raise TypeError("id is not a int")
            if id < 0:
                raise CephError(cmd="osd_create",
                                msg=str(id) + " is less than min of 0")
            cmd['id'] = id
    ...
    def osd_tier_cache_mode(self, mode, pool, sure=None):
        """
    ...
    def osd_tier_add_cache(self, tierpool, pool, size):
        """
